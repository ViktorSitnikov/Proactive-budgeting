...
export function InitiatorHome({ user, onLogout }: InitiatorHomeProps) {
  ...
  const { resetData, setFromDraft, updateData } = useApplicationStore()
  ...
  useEffect(() => {
    const handleNavigate = async (e: Event) => {
      ...
      if (project) {
        ...
        try {
          const fetched = await projectsApi.getProjectById(projectId);
    ...
    window.addEventListener('NAVIGATE_TO_PROJECT', handleNavigate);
    return () => window.removeEventListener('NAVIGATE_TO_PROJECT', handleNavigate);
  }, [projects]);

  const handleCreateNew = useCallback(async () => {
    try {
      const draft = await projectsApi.saveDraft({
        title: "Новая заявка",
        description: "",
        step: 1,
      ...
      resetData()
      setFromDraft(draft)
      updateData({ id: draft.id, title: draft.title, idea: draft.description || "" })
  ...
  }, [resetData, setFromDraft, updateData, queryClient])

  const handleContinueDraft = useCallback((draft: Draft) => {
    setFromDraft(draft)
    const step = Math.min(4, Math.max(1, draft.step ?? 1))
    setInitialCreateStep(step)
    setCurrentPage("create")
  }, [setFromDraft])
    ...
    return (
      <CreateApplicationPage
        ...
        onExitToApplicationsList={() => {
          void queryClient.invalidateQueries({ queryKey: ["drafts"] })
          setCurrentPage("applications-list")
        }}
        initialStep={initialCreateStep as 1 | 2 | 3 | 4 | 5 | "ai_loading"}
      />
    )
  }
