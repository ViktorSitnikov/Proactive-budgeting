        ...
        {isLoading ? (
          ...
          <div className="space-y-4 md:space-y-6">
            {projects.map((project) => {
              ...
              return (
                <Card key={project.id} className={`group hover:shadow-2xl transition-all border-none shadow-lg overflow-hidden mx-2 md:mx-0 ${hasDirectRequest ? 'ring-2 ring-primary shadow-primary/20' : ''}`}>
                  <CardHeader className="bg-white border-b border-slate-50 p-4 md:p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          {hasDirectRequest && (
                            <Badge className="bg-primary text-primary-foreground border-none animate-pulse text-[10px]">Персональный запрос</Badge>
                          )}
                          <Badge variant="outline" className="text-[8px] md:text-[10px] uppercase font-bold tracking-wider border-slate-200">
                            ID: {project.id.slice(0, 8)}
                          </Badge>
                        </div>
                        <CardTitle className="text-lg md:text-2xl font-black text-slate-900 group-hover:text-primary transition-colors line-clamp-2">
                          {project.title}
                        </CardTitle>
                        <div className="flex flex-wrap items-center gap-x-4 md:gap-6 mt-3 md:mt-4 text-[10px] md:text-sm font-bold text-slate-400">
                          <div className="flex items-center gap-1 md:gap-2">
                            <MapPin className="w-3 md:w-4 h-3 md:h-4 text-primary shrink-0" />
                            <span className="truncate">{project.location}</span>
                          </div>
                          <div className="flex items-center gap-1 md:gap-2">
                            <Coins className="w-3 md:w-4 h-3 md:h-4 text-primary shrink-0" />
                  ...
                  <CardContent className="p-4 md:p-6 bg-white">
                    {hasDirectRequest && directRequestMessage && (
                      <div className="bg-primary/10 border-l-4 border-primary p-3 md:p-4 mb-4 md:mb-6 rounded-r-xl">
                        <p className="text-xs md:text-sm font-medium text-foreground italic">
