...
export function CurrentProjectsNPOPage({ user, onBack, onLogout }: CurrentProjectsNPOPageProps) {
  ...
  const { data: projects = [], isLoading } = useQuery({
    queryKey: ['projects', 'npo', user.id],
    queryFn: async () => await projectsApi.getProjects({ npoId: user.id }),
    select: (data) => data.filter((p) => p.status !== ProjectStatuses.success)
  })
        ...
        {isLoading ? (
            ...
            {projects.map((project) => (
              ...
              >
                <div className="aspect-video bg-muted relative overflow-hidden">
                  ...
                  <Badge className="absolute top-4 right-4 bg-primary text-primary-foreground border-none px-3 py-1 font-bold uppercase text-[10px] tracking-widest">
                    В реализации
                  </Badge>
          ...
          <div className="text-center py-24 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 mb-8 max-w-sm mx-auto">
              Выберите проект в разделе «Запросы на партнерство», чтобы начать работу
            </p>
